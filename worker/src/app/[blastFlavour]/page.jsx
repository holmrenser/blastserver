...
const DB_NAMES = new Map([...PROTEIN_DBS, ...NUCLEOTIDE_DBS]);
const BLAST_DBS = new Map([
...
const formSchema = Yup.object().shape({
    ...
    query: Yup.string()
        ...
        .max(10e4)
        .trim()
        .test('is-not-multifasta', 'Query contains multiple FASTA sequences', (value, context) => (value.match(/>/g) || []).length < 2)
        .test('is-not-short', 'Query is shorter than 25 characters', (value, context) => value.length >= 25)
        .test('is-not-long', 'Query is longer than 10,000 characters', (value, context) => value.length <= 10000),
    ...
    return (<fieldset className='box'>
        ...
        <div className='field-body'>
          <div className='field'>
            ...
            <div className="control">
              <textarea className={`textarea is-small ${((_a = errors.query) === null || _a === void 0 ? void 0 : _a.message) ? 'is-danger' : ''}`} placeholder="QUERY SEQUENCE" {...register('query')}/>
            </div>
            {errors.query && (<p className='help is-danger'>
                  {String((_b = errors.query) === null || _b === void 0 ? void 0 : _b.message)}
                </p>)}
          </div>
          ...
          <div className='field'>
            ...
            <div className='field is-horizontal'>
              ...
              <div className='field-body'>
                <div className='field'>
                  <div className="control">
                    <input className={`input is-small ${((_c = errors.queryFrom) === null || _c === void 0 ? void 0 : _c.message) ? 'is-danger' : ''}`} placeholder="FROM" style={{ maxWidth: 120 }} {...register('queryFrom')}/>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className='field is-horizontal' style={{ paddingTop: '.75em' }}>
              ...
              <div className='field-body'>
                <div className='field'>
                  <div className="control">
                    <input className={`input is-small ${((_d = errors.queryTo) === null || _d === void 0 ? void 0 : _d.message) ? 'is-danger' : ''}`} placeholder="TO" type='text' style={{ maxWidth: 120 }} {...register('queryTo')}/>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className='field is-horizontal'>
        ...
        <div className="field-body">
          <div className="field">
            <div className="control">
              <input className="input is-small" type="text" placeholder="JOBTITLE" style={{ maxWidth: 240 }} disabled {...register('jobTitle')}/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className='field is-horizontal'>
        ...
        <div className="field-body">
          <div className="field">
            <div className="control">
              <input className="input is-small" type="text" placeholder="JOHN@DOE.COM" style={{ maxWidth: 240 }} disabled {...register('email')}/>
            </div>
          </div>
        </div>
      </div>
...
function ChooseSearchSet({ register, errors, blastFlavour, control }) {
    var _a;
    ...
    return (<fieldset className='box'>
      ...
      <div className='field is-horizontal'>
        ...
        <div className="field-body">
          <div className="field">
            <div className="control">
              <div className={`select is-small ${((_a = errors.database) === null || _a === void 0 ? void 0 : _a.message) ? 'is-danger' : ''}`}>
                <select {...register('database')} style={{ minWidth: 400 }}>
                  {dbOptions && dbOptions.map(db => (<option key={db} value={db}>
                        {`${DB_NAMES.get(db)} (${db})`}
                      </option>))}
                </select>
              </div>
    ...
    return (<fieldset className='box'>
      ...
      <div className='field is-horizontal'>
        ...
        <div className="field-body">
          <div className="field">
            <div className="control">
              {(_a = PROGRAMS.get(blastFlavour)) === null || _a === void 0 ? void 0 : _a.map((program) => (<React.Fragment key={program}>
                  <label className="radio is-small">
                    <input type="radio" checked={program === selectedProgram} {...register('program')}/>
                    &nbsp;
                    {program}
                  </label>
                  <br />
      ...
      <div className='panel-block algorithm-parameters'>
              ...
              <div className="field">
                ...
                <div className="select is-small">
                  <select {...register('maxTargetSeqs')}>
                    {[10, 50, 100, 250, 500, 1000, 5000].map(n_targets => (<option key={n_targets}>{n_targets}</option>))}
                  </select>
                </div>
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="select is-small">
                  <select {...register('wordSize')}>
                    {[2, 3, 5, 6].map(wordSize => (<option key={wordSize}>{wordSize}</option>))}
                  </select>
                </div>
                </div>
              </div>
        ...
        <fieldset className='box'>
          ...
          <div className='field is-horizontal'>
            ...
            'BLOSUM62', 'BLOSUM45', 'BLOSUM50', 'BLOSUM90'].map(wordSize => (<option key={wordSize}>{wordSize}</option>))}
                  </select>
                </div>
                </div>
              </div>
          ...
          <div className='field is-horizontal'>
            ...
            'Universal compositional score matrix adjustment'].map(adjustment => (<option key={adjustment}>{adjustment}</option>))}
                  </select>
                </div>
                </div>
              </div>
...
export default function BlastFlavourPage({ params }) {
    ...
    const { register, handleSubmit, getValues, formState: { errors }, control } = useForm({
        ...
        defaultValues: {
            ...
            database: BLAST_DBS.get(blastFlavour)[0],
            maxTargetSeqs: 100,
            shortQueries: true,
